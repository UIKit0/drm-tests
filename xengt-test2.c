...
main(int argc, char **argv)
{
    ...
    struct drm_i915_gem_vgtbuffer create;
	...
	printf ("Checking mode %s == 1680x1050\n", mode->name);
	...
	create.plane_id = 1;
	create.pipe_id = 0;
	create.user_size = ((1680 * 4 * 1050) + 4095) & ~4095;
        buf = (uint32_t *) malloc(create.user_size);
        memset(buf, 0xFF, create.user_size);
        create.user_ptr = (uint64_t) buf;
        create.flags = I915_VGTBUFFER_READ_ONLY;


        printf("Requesting object for VM %d\n", create.vmid);

        if (ret = drmIoctl(fd, DRM_IOCTL_I915_GEM_VGTBUFFER, &create)) {
		printf("drmIoctl failed for domain %d. ret = %d errno = %d\n", create.vmid, ret, errno);
		continue;
	}
        handle[i] = create.handle;
        stride[i] = create.stride;

	printf ("Creating KMS framebuffer for VM%d: handle = %d, stride = %d\n", create.vmid, handle[i], stride[i]);
    ...
    do {
        ...
        drmSetMaster(fd);
    	printf("setting crtc for vm to %s\n", mode->name);
