...
main(int argc, char **argv)
{
    ...
    struct drm_i915_gem_foreign create;
	...
	int ret;
        create.domid = atoi(argv[i+1]);
	create.size = 1366*768*4;
	create.gfn = 0xC0000000;

        printf("Requesting object for VM %d\n", create.domid);

        if (ret = drmIoctl(fd, DRM_IOCTL_I915_GEM_FOREIGN, &create)) {
        ...
        if (ret = drmModeAddFB(fd, mode->hdisplay, mode->vdisplay, 32, 32, stride[i], handle[i], &fb_id[i])) {
